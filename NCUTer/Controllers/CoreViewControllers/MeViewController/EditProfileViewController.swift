...
class EditProfileViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    private let genderPickerToolBar:UIToolbar = {
        ...
        let doneButton = UIBarButtonItem(title: "完成", style: UIBarButtonItem.Style.done, target: self, action: #selector(doneSexPicker))
        let spaceButton = UIBarButtonItem(barButtonSystemItem: UIBarButtonItem.SystemItem.flexibleSpace, target: nil, action: nil)
        let cancelButton = UIBarButtonItem(title: "取消", style: UIBarButtonItem.Style.plain, target: self, action: #selector(doneSexPicker))
    ...
    private let birthdayPickerToolBar:UIToolbar = {
        ...
        let doneButton = UIBarButtonItem(title: "完成", style: UIBarButtonItem.Style.done, target: self, action: #selector(doneBirthdayPicker))
        let spaceButton = UIBarButtonItem(barButtonSystemItem: UIBarButtonItem.SystemItem.flexibleSpace, target: nil, action: nil)
        let cancelButton = UIBarButtonItem(title: "取消", style: UIBarButtonItem.Style.plain, target: self, action: #selector(doneBirthdayPicker))
    ...
    private func configureModels(){
        ...
        DatabaseManager.shared.getUser(user:LCApplication.default.currentUser!){[weak self] user in
            ...
            DispatchQueue.main.async {
                self?.data.append(EditProfileCellModel(title: "性别",value: gender2){
                    [weak self] in
                    self?.didTapGender()
                }
                )
                self?.data.append(EditProfileCellModel(title: "昵称",value: nickname){
                    [weak self] in
                    self?.didTapGender()
                }
                )
                self?.data.append(EditProfileCellModel(title: "生日",value: birthday){
                    [weak self] in
                    self?.didTapGender()
                }
                )
                self?.data.append(EditProfileCellModel(title: "个性签名",value: bio){
                    [weak self] in
                    self?.didTapGender()
                }
                )
