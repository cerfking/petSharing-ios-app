        ...
        if username.contains("@"),username.contains("."){
            AuthManager.shared.registerNewUserWithUsername(username: username, password: password){ [weak self]
                ...
                DispatchQueue.main.async {
                    if registered{
                        let successAlert = UIAlertController(title: nil, message: "注册成功", preferredStyle: .alert)
                        successAlert.addAction(UIAlertAction(title: "好", style: .cancel, handler: nil))
                        self?.present(successAlert, animated: true, completion: nil)
                    }
                    else{
                        let failAlert = UIAlertController(title: "注册失败", message: "用户已存在", preferredStyle: .alert)
                        failAlert.addAction(UIAlertAction(title: "好", style: .cancel, handler: nil))
            ...
            let failAlert = UIAlertController(title: "注册失败", message: "请输入正确的邮箱", preferredStyle: .alert)
            failAlert.addAction(UIAlertAction(title: "好", style: .cancel, handler: nil))
